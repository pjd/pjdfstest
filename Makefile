...
${PROG}:	${PROG}.c
	...
	if [ $$OSTYPE = "FreeBSD" ]; then \
		OS_CFLAGS="$$OS_CFLAGS -DHAS_LCHMOD -DHAS_CHFLAGS -DHAS_FCHFLAGS -DHAS_LCHFLAGS -DHAS_POSIX_FALLOCATE -DHAS_UTIMENSAT ${HAS_FREEBSD_ACL}"; \
	elif [ $$OSTYPE = "SunOS" ]; then \
		OS_CFLAGS="$$OS_CFLAGS -DHAS_TRUNCATE64 -DHAS_STAT64"; \
		OS_LDLIBS="$$OS_LDLIBS -lsocket"; \
	elif [ $$OSTYPE = "Darwin" ]; then \
		OS_CFLAGS="$$OS_CFLAGS -DHAS_BIRTHTIME -DHAS_LCHMOD -DHAS_CHFLAGS -DHAS_LCHFLAGS"; \
	elif [ $$OSTYPE = "Linux" ]; then \
		OS_CFLAGS="$$OS_CFLAGS -D_GNU_SOURCE -DHAS_UTIMENSAT"; \
	else \
